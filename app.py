...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if st.session_state.get('layer_data'):
            ...
            with st.expander("📊 Analysis Controls", expanded=True):
                ...
                is_still_alive_view = view_mode == ViewMode.STILL_ALIVE.value
            ...
            with st.expander("📥 Reporting", expanded=True):
                ...
                st.download_button(
                    ...
                    file_name=f"defect_report_layer_{st.session_state.selected_layer}.xlsx",
    ...
    if st.session_state.get('layer_data'):
        ...
        if view_mode == ViewMode.STILL_ALIVE.value:
            ...
            fig.update_layout(
                title=dict(text=f"Still Alive Map ({defective_cell_count} Defective Cells)", font=dict(color=TEXT_COLOR), x=0.5, xanchor='center'),
                ...
                plot_bgcolor=PLOT_AREA_COLOR, paper_bgcolor=BACKGROUND_COLOR, shapes=map_shapes, height=800, margin=dict(l=20, r=20, t=60, b=20)
            )
        ...
        if layer_keys:
            ...
            for i, layer_num in enumerate(layer_keys):
                with cols[i]:
                    if st.button(f"Layer {layer_num}", key=f"layer_btn_{layer_num}", use_container_width=True, type="primary" if st.session_state.selected_layer == layer_num else "secondary"):
        ...
        if view_mode == ViewMode.DEFECT.value:
            ...
            fig.update_layout(
                ...
                plot_bgcolor=PLOT_AREA_COLOR, paper_bgcolor=BACKGROUND_COLOR, legend=dict(title_font=dict(color=TEXT_COLOR), font=dict(color=TEXT_COLOR)), height=600
            )
            st.plotly_chart(fig, use_container_width=True)
            ...
            if quadrant_selection != Quadrant.ALL.value:
                ...
                if not display_df.empty:
                    ...
                    fig.update_layout(
                        ...
                        barmode='stack', xaxis=dict(title="Defect Type & Quadrant", title_font=dict(color=TEXT_COLOR), tickfont=dict(color=TEXT_COLOR)),
                        yaxis=dict(title="Count", title_font=dict(color=TEXT_COLOR), tickfont=dict(color=TEXT_COLOR)),
                        plot_bgcolor=PLOT_AREA_COLOR, paper_bgcolor=BACKGROUND_COLOR, legend=dict(title="Verification Status", font=dict(color=TEXT_COLOR)), height=600
                    )
                    st.plotly_chart(fig, use_container_width=True)
                else:
